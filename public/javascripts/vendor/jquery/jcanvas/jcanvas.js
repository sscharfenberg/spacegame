	...
	for (eventName in jCanvas.events) {
		if (jCanvas.events.hasOwnProperty(eventName)) {
			...
			if (layer[eventName] || (layer.cursors && layer.cursors[eventName])) {
				// Bind event to layer
				_addLayerEvent($mapcanvas, data, layer, eventName);
			}
		}
	}
}
...
function _addLayerEvent($canvas, data, layer, eventName) {
	...
	// Bind event to layer
	jCanvas.events[eventName]($mapcanvas, data);
	layer._event = TRUE;
}
...
function _enableDrag($canvas, data, layer) {
	...
	if (layer.draggable || layer.cursor || layer.cursors) {
		...
		if (!data.events.mouseoutdrag) {
			$mapcanvas.bind('mouseout.jCanvas', function() {
				...
				if (layer) {
					...
					if (layer.dragcancel) {
						layer.dragcancel.call($mapcanvas[0], layer);
					}
...
$.fn.setLayer = function setLayer(layerId, props) {
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		if (layer) {
			...
			_updateLayerName($mapcanvas, data, layer, props);
			_updateLayerGroups($mapcanvas, data, layer, props);

			// If index was given
			if (props.index !== UNDEFINED) {
				// Move layer to that new index
				$mapcanvas.moveLayer(layer, props.index);
			}
			...
			// Update layer events
			_addLayerEvents($mapcanvas, data, layer);
			_enableDrag($mapcanvas, data, layer);
...
$.fn.setLayerGroup = function setLayerGroup(groupId, props) {
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		group = $mapcanvas.getLayerGroup(groupId);
		// If group exists
		if (group) {
			...
			for (l = 0; l < group.length; l += 1) {
				...
				$mapcanvas.setLayer(group[l], props);
...
$.fn.setLayers = function setLayers(props, callback) {
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		layers = $mapcanvas.getLayers(callback);
		// Loop through all layers
		for (l = 0; l < layers.length; l += 1) {
			// Set properties of each layer
			$mapcanvas.setLayer(layers[l], props);
		}
	}
	return $canvases;
};
...
$.fn.moveLayer = function moveLayer(layerId, index) {
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		layers = $mapcanvas.getLayers();
		layer = $mapcanvas.getLayer(layerId);
		if (layer) {
...
$.fn.removeLayer = function removeLayer(layerId) {
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		if (layer) {
			...
			_updateLayerName($mapcanvas, data, layer, {
				name: NULL
			});
			// Update layer group map
			_updateLayerGroups($mapcanvas, data, layer, {
				groups: NULL
			});
...
function _drawLayer($canvas, ctx, layer, nextLayerIndex) {
	if (layer && layer.visible && layer._method) {
		...
		layer._method.call($mapcanvas, layer);
	}
}
...
$.fn.drawLayer = function drawLayer(layerId) {
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		layer = $mapcanvas.getLayer(layerId);
		_drawLayer($mapcanvas, ctx, layer);
	}
	return $canvases;
};
...
function _handleLayerDrag($canvas, data, eventType) {
	...
	if ((eventType === 'mousemove' || eventType === 'touchmove')) {
		...
		if (!drag.dragging) {
					...
					for (l = 0; l < group.length; l += 1) {
						if (group[l] !== layer) {
							...
							if (group[l].dragstart && layer.fireDragGroupEvents) {
								group[l].dragstart.call($mapcanvas[0], group[l]);
							}
			...
			triggerLayerEvent($mapcanvas, layer, 'dragstart');
				...
				for (l = 0; l < group.length; l += 1) {
					if (group[l] !== layer) {
						...
						if (group[l].drag && layer.fireDragGroupEvents) {
							group[l].drag.call($mapcanvas[0], group[l]);
						}
					}
				}
		...
		// Trigger drag event if defined
		triggerLayerEvent($mapcanvas, layer, 'drag');
		...
		if (drag.dragging) {
			triggerLayerEvent($mapcanvas, layer, 'dragstop');
			drag.dragging = FALSE;
		}
				...
				for (l = 0; l < group.length; l += 1) {
					if (group[l] !== layer) {
						...
						if (group[l].dragstop && layer.fireDragGroupEvents) {
							group[l].dragstop.call($mapcanvas[0], group[l]);
						}
					}
				}
			}
...
function triggerLayerEvent($canvas, layer, eventType, cursor) {
	...
	if (layer[eventType]) {
		layer[eventType].call($mapcanvas[0], layer);
	}
}
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		if (ctx) {
			...
			for (l = params.index; l < layers.length; l += 1) {
				...
				// Draw layer
				_drawLayer($mapcanvas, ctx, layer, l + 1);
			...
			if (lastLayer !== NULL && layer !== lastLayer && lastLayer._hovered && !lastLayer._fired) {
				...
				triggerLayerEvent($mapcanvas, lastLayer, 'mouseout', data.cursor);
			...
			if (layer._event && layer.intersects) {
				...
				if (layer.mouseover || layer.mouseout || layer.cursor || layer.cursors) {
					...
					if (!layer._hovered && !layer._fired) {
						...
						triggerLayerEvent($mapcanvas, layer, 'mouseover');
				...
				if (!layer._fired) {
					...
					triggerLayerEvent($mapcanvas, layer, eventType);
			...
			if (drag.layer) {
				_handleLayerDrag($mapcanvas, data, eventType);
			}
	...
	*/
	...
	function step($canvas, data, layer) {
		...
		return function(now, fx) {
			...
			if (layer._pos !== fx.pos) {
				...
				if (args[5]) {
					args[5].call($mapcanvas[0], now, fx, layer);
				}
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		if (ctx) {
			...
			layer = $mapcanvas.getLayer(args[0]);
			...
			if (layer && layer._method !== $.fn.draw) {
				...
				$(layer).animate(props, {
					...
					complete: complete($mapcanvas, data, layer),
					// Redraw canvas for every animation frame
					step: step($mapcanvas, data, layer)
				});
			}
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		if (group) {
			...
			for (l = 0; l < group.length; l += 1) {
				...
				$mapcanvas.animateLayer.apply($mapcanvas, [group[l]].concat(args.slice(1)));
...
function createEvent(eventName) {
	...
	jCanvas.events[eventName] = function($canvas, data) {
		...
		if (!data.events[helperEventName]) {
			...
			$mapcanvas.bind(helperEventName + '.jCanvas', function(event) {
